...
curl -fsS "http://${API_DOMAIN}/.well-known/acme-challenge/ping-api" >/dev/null
curl -fsS "http://${WP_DOMAIN}/.well-known/acme-challenge/ping-wp" >/dev/null
echo "ACME probe OK"
...
for line in p.read_text().splitlines():
    if line.startswith("APP_BASE_URL=") or line.startswith("PUBLIC_BASE_URL="):
        ...
        lines.append(line)
p.write_text("\n".join(lines)+"\n")
print("env:", [l for l in lines if l.startswith("APP_BASE") or l.startswith("PUBLIC_BASE")])
PY
...
curl -sS -m 15 -o /dev/null -w "api https://staging.seoauto.vn/docs => %{http_code}\n" "https://${API_DOMAIN}/docs"
curl -sS -m 15 -o /dev/null -w "wp  https://wp-staging.seoauto.vn/ => %{http_code}\n" "https://${WP_DOMAIN}/"
